...
const DEFAULTS: Required<CompactionOptions> = {
  ...
  // keepRecent) makes the model lose the thread of the current task. 18 recent
  // turns preserves the active back-and-forth while still bounding growth.
  keepRecent: 18,
...
export function compactHistory(messages: Message[], options: CompactionOptions = {}): CompactionResult {
  ...
  for (const m of middle) {
    // Drop pure status/no-op user nudges that don't carry tool results.
    if (
      m.role === 'user' &&
      typeof m.content === 'string' &&
      m.content.length < 400 &&
      /(continue|please continue|what should we do next)/i.test(m.content)
    ) {
      dropped++;
      continue;
    }
    compactedMiddle.push(compactMessage(m, Math.floor(opts.maxToolResultChars / 2)));
  }

  // If still over budget, aggressively shrink the oldest compacted-middle items.
