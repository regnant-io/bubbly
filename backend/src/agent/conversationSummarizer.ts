 ...
 */
function deterministicSummary(messages: Message[], extraContext?: string): string {
  ...
  return [
    ...
    ``,
    ...(extraContext
      ? [`### Environment already set up`, extraContext, ``]
      : []),
    `### Note`,
...
Reproduce the "Live environment state" block from the input VERBATIM, as a bullet list. Do not summarize it, do not soften it, do not judge whether it is still relevant. This is what is running, installed and configured RIGHT NOW.

This section is the whole reason handoffs go wrong when it is missing: the next engineer starts a dev server that never stopped, or reinstalls dependencies that are already there, because a prose summary told them what HAPPENED and not what IS. If the input gives you no such block, write "Nothing reported."

## Gotchas
Constraints, failed approaches to avoid, and anything subtle the next engineer must know.
...
Be dense and factual. No fluff. If something is unknown, say so rather than guessing.`;
  ...
  const userPrompt =
    ...
    (params.extraContext
      ? `Live environment state (observed just now — carry this into the "Environment already set up" section word for word):\n${params.extraContext}\n\n`
      : '') +
  ...
  try {
    ...
    if (text.length < 40) {
      ...
      return { text: deterministicSummary(messages, params.extraContext), modelGenerated: false };
    }
