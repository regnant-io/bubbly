...
export interface WorkspaceIndex {
  ...
  fileCount: number;
  /**
   * The walk hit a budget before it finished the tree.
   *
   * Surfaced rather than swallowed: an agent told "here is the codebase" when
   * it has been shown a fifth of it will confidently conclude that a file does
   * not exist. The repo map says so in as many words — see buildRepoMap.
   */
  truncated?: boolean;
  truncationReason?: 'file-budget' | 'time-budget';
}

/**
 * Directories never worth indexing.
 *
 * These are DEPENDENCIES AND OUTPUT, not source. Walking them is the single
 * largest cost in a big repository — a Cocoapods checkout, a Rust `target/` or
 * a Bazel output tree can be an order of magnitude more files than the project
 * itself — and nothing in them is what someone is asking the agent about.
 *
 * The list is long on purpose. A directory missing from it does not merely slow
 * the index down; it dilutes the repo map with generated code, which then
 * crowds out the files the model actually needed to see.
 */
const DEFAULT_EXCLUDES = [
  // Dependencies
  'node_modules', 'vendor', 'bower_components', 'Pods', '.pnpm-store', '.yarn',
  'third_party', 'thirdparty', 'external',
  // Python
  '__pycache__', '.pytest_cache', '.venv', 'venv', 'env', '.tox', '.mypy_cache',
  ...
  'bazel-out', 'bazel-bin', 'bazel-testlogs', 'cmake-build-debug', 'DerivedData',
  // Caches, tooling and VCS
  '.git', '.hg', '.svn', '.cache', '.turbo', '.gradle', '.m2', '.terraform',
  'coverage', '.nyc_output', '.idea', '.vscode', '.bubbly', '.history',
  // Generated
  'generated', '__generated__', '.generated', 'migrations/versions',
];

/**
 * HOW BIG A REPOSITORY THIS WILL INDEX.
 *
 * Indexing is synchronous: a directory walk, a stat per file, and a tree-sitter
 * parse of everything that changed. On a normal project that is a few hundred
 * milliseconds and nobody notices. On a monorepo with fifty thousand source
 * files it is minutes of BLOCKED EVENT LOOP — the HTTP server stops answering,
 * the WebSocket stops streaming, and the app looks hung, on the first message
 * of every session.
 *
 * So there is a budget, and when it is exceeded the index is PARTIAL AND SAYS
 * SO rather than either hanging or silently pretending the rest of the tree
 * does not exist. The files kept are the ones discovered first, which after the
 * exclusions above is overwhelmingly the project's own source; `search`,
 * `read_file` and `get_file_tree` are unaffected and still see everything, so
 * the agent can always reach a file the index skipped — it just does not get a
 * free structural summary of it.
 */
...
export interface DiscoveryResult {
  files: Array<{ rel: string; full: string; size: number; mtimeMs: number }>;
  /** True when the budget stopped the walk before it finished the tree. */
  truncated: boolean;
  /** Why it stopped, for the log and for the repo map's footer. */
  reason?: 'file-budget' | 'time-budget';
}

function discoverCodeFiles(workspacePath: string): DiscoveryResult {
   ...
   *
   * When a budget cuts the walk short, WHICH files were found matters as much
   * as how many. Depth-first spends the whole budget inside the first
   * subdirectory it happens to enter — so a monorepo would index all of
   * `packages/a` in exhaustive detail and nothing else, and the repo map would
   * describe one twentieth of the project as though it were the whole thing.
   * Breadth-first spends it evenly across the tree, so a truncated index is a
  ...
  while (queue.length > 0) {
    ...
    for (const e of entries) {
      ...
      if (e.isDirectory()) {
        // A symlinked directory can point back up the tree; following one turns
        // the walk into an infinite loop that the budgets would only ever cut
        // short rather than prevent.
  ...
  if (truncated) {
    logger.warn('Code index truncated by budget', {
      workspacePath, files: out.length, reason, elapsedMs: Date.now() - startedAt,
    });
  }
...
export function buildIndex(workspacePath: string): WorkspaceIndex {
  ...
  const t0 = Date.now();
  const discovery = discoverCodeFiles(abs);
  const discovered = discovery.files;
  const fileSet = new Set<string>(discovered.map((d) => d.rel));
...
/**
 * How long a built index is trusted before the tree is re-walked.
 *
 * Thirty seconds is right for an ordinary project, where a rebuild is a
 * directory walk and a handful of re-parses — a few tens of milliseconds, and
 * being current matters more than the cost.
 *
 * It is exactly wrong for a very large one. There, the walk alone is seconds of
 * BLOCKED EVENT LOOP, and re-walking every thirty seconds means a long agent
 * turn spends a meaningful fraction of its life stat-ing a tree that has barely
 * changed — while the WebSocket it is supposed to be streaming into waits. The
 * agent's own writes call invalidateIndex(), so its changes are picked up
 * immediately regardless; this TTL only governs how often we go looking for
 * changes made by SOMEONE ELSE, and on a repository that big, five minutes is a
 * perfectly good answer to that.
 */
const INDEX_TTL_MS = 30_000;
const LARGE_INDEX_TTL_MS = 5 * 60_000;
/** Above this many files, a rebuild costs more than staleness does. */
const LARGE_INDEX_FILES = 4_000;
...
export function getIndex(workspacePath: string, forceRebuild = false): WorkspaceIndex {
  ...
  if (!forceRebuild) {
    ...
    if (cached) {
      const ttl = cached.truncated || cached.fileCount >= LARGE_INDEX_FILES ? LARGE_INDEX_TTL_MS : INDEX_TTL_MS;
      if (Date.now() - cached.builtAt < ttl) return cached;
    }
  }
...
export function buildRepoMap(workspacePath: string, opts: RepoMapOptions = {}): string {
  ...
  if (index.truncated) {
    // SAY SO. An agent handed a map of a fifth of a monorepo, with no warning
    // that it is a fifth, will state that a file does not exist rather than
    // going to look for it — and it will be confident, because the map was
    // presented as the codebase.
    lines.push(
      `NOTE: this repository is larger than the index budget, so the map above covers ` +
      `${index.fileCount} files and NOT the whole tree ` +
      `(${index.truncationReason === 'time-budget' ? 'the walk ran out of time' : 'the file budget was reached'}). ` +
      `Absence from this map does NOT mean a file does not exist — use search / get_file_tree / read_file, ` +
      `which see the entire workspace.`,
    );
  }
  lines.push('');
