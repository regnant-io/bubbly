...
const LEAD_TOOL_NAMES = new Set<string>([
  ...
  'update_plan',
  // Naming the phase of the work. Available in EVERY mode, including spec
  // sessions where update_plan is not — a spec session runs the longest bursts
  // of all, and is exactly where an unlabelled wall of steps is least readable.
  'set_phase',
  'delegate_task',
  'delegate_parallel',
  'ask_user',
...
const DIRECT_WORK_TOOL_NAMES = new Set<string>([
  ...
  // Repository work: the working tree, and the hosted side of it.
  'repo', 'forge',
...
/**
 * MESSAGES THE USER SENDS WHILE THE AGENT IS ALREADY WORKING.
 *
 * A run is single-flight — two loops over one thread duplicate every tool call —
 * so a second `chat` for a busy thread used to be refused outright. That is the
 * right protection and the wrong experience: the moment you most want to say
 * something ("no, not that file", "also add the tests") is the moment you are
 * watching the agent do it, and being told to wait means either sitting on your
 * hands or pressing Stop and losing the turn.
 *
 * So the message is QUEUED against the thread and handed to the running loop at
 * its next safe boundary — between iterations, where a plain user message is
 * exactly what the history expects. It is bounded at three, because a queue
 * deeper than that stops being "one more thought" and becomes a second
 * conversation the agent will answer all at once, badly.
 */
const MAX_QUEUED_MESSAGES = 3;
const queuedMessages = new Map<string, string[]>();

export function queueUserMessage(sessionId: string, message: string):
  { ok: true; queued: number } | { ok: false; error: string; queued: number } {
  const text = message.trim();
  const queue = queuedMessages.get(sessionId) ?? [];
  if (!text) return { ok: false, error: 'An empty message cannot be queued.', queued: queue.length };
  if (queue.length >= MAX_QUEUED_MESSAGES) {
    return {
      ok: false,
      queued: queue.length,
      error: `Up to ${MAX_QUEUED_MESSAGES} messages can wait while the agent works, and ${queue.length} already are. They are delivered together at the next step.`,
    };
  }
  queue.push(text);
  queuedMessages.set(sessionId, queue);
  logger.info('Queued a message for a running thread', { sessionId, depth: queue.length });
  return { ok: true, queued: queue.length };
}

/** How many messages are waiting — for the UI's chip and for `stop`. */
export function queuedMessageCount(sessionId: string): number {
  return queuedMessages.get(sessionId)?.length ?? 0;
}

/** Take everything waiting, leaving the queue empty. */
function takeQueuedMessages(sessionId: string): string[] {
  const queue = queuedMessages.get(sessionId);
...
/** Drop anything waiting — the user stopped the run, so it was never sent. */
export function clearQueuedMessages(sessionId: string): string[] {
  return takeQueuedMessages(sessionId);
}
...
export function stopSession(sessionId: string): void {
  ...
  // Anything the user typed while the agent worked was queued for a step that
  // is now never going to happen. Returning it lets the caller put the text
  // back in the composer instead of silently eating it.
  const unsent = clearQueuedMessages(sessionId);
  if (unsent.length > 0) {
    logger.info('Discarded queued messages for a stopped session', { sessionId, count: unsent.length });
  }
...
/** What was queued but never delivered, so a Stop can hand it back. */
export function stopSessionReturningQueue(sessionId: string): string[] {
  const pending = queuedMessages.get(sessionId)?.slice() ?? [];
  stopSession(sessionId);
  return pending;
}

/**
 * HOW LONG A PENDING APPROVAL WAITS FOR A HUMAN.
 *
 * This was five minutes, and five minutes is the length of a coffee. The
 * failure mode is quiet and expensive: you step away, the agent asks to run a
 * migration, nobody answers, the request is recorded as DECLINED, and the agent
 * carries on around the thing it was not allowed to do — so you come back to a
 * finished turn that silently skipped the important step.
 *
 * Half an hour is long enough to cover stepping away from the desk and short
 * enough that a forgotten window does not pin a session open all night. Nothing
 * is lost by waiting: the run is parked, Stop still works instantly, and with
 * Bubbly living in the system tray the window being closed is no longer a
 * reason to assume nobody is coming back.
 *
 * An EXPIRY is still recorded distinctly from a rejection. "A human looked at
 * this and said no" and "nobody was watching" call for different responses, and
 * an audit log that cannot tell them apart is not an audit log.
 */
const APPROVAL_TIMEOUT_MS = 30 * 60_000;

/**
 * Request human approval for a tool call. Persists the pending approval, emits
 * the UI events, and resolves when the user decides (or after
 * APPROVAL_TIMEOUT_MS → declined). Shared by the main loop and the spec task
 * agents.
 */
...
- **Break down by concern, not by file count**: Separate data model changes from business logic from UI from tests. For example:
  - T1: Add database schema for feature X (migration + types)
  - T2: Implement core business logic for feature X (service layer + unit tests)
  - T3: Add API endpoints for feature X (routes + integration tests)
  - T4: Build UI components for feature X (components + UI tests)
  NOT: "T1: Implement feature X" (that's 4 tasks stacked into one)
- **Infrastructure first**: Database schema, types, core utilities come before features that depend on them
...
- **Each task includes its own verification**: Tests for that task, type-checking, and any specific checks (browser test, API call, etc.)
- Include the unglamorous work explicitly: migrations, error handling, input validation, loading states, empty states, edge case handling, wiring/registration, config, docs the change invalidates. Work that isn't a task doesn't get done.
- **Production-ready means**: Error handling, input validation, proper logging, edge cases covered, tests written, accessibility checked
...
- Shell for run_command: ${process.platform === 'win32' ? 'cmd.exe (Windows Command Prompt)' : 'sh/bash'}
${process.platform === 'win32'
  ? `- You are on WINDOWS using cmd.exe. Use CMD commands, NOT PowerShell or Unix commands. Examples: use \`dir\` (not \`ls\` or \`Get-ChildItem\`), \`type\` (not \`cat\` or \`Get-Content\`), \`del\` (not \`rm\` or \`Remove-Item\`), \`copy\` (not \`cp\` or \`Copy-Item\`), \`mkdir\` (not \`mkdir -p\` or \`New-Item\`), \`findstr\` (not \`grep\` or \`Select-String\`). Use \`&&\` or \`&\` to chain commands. Environment variables use \`%VAR%\` syntax (not \`$env:VAR\`). Use backslashes or forward slashes in paths.`
...
## Say what you are doing, as you change what you are doing
Call **set_phase("...")** immediately before the first tool call of each new stretch of work — four or five words naming the WORK, not the tool: "Building the backend", "Tracking down the failing test", "Fixing the missing import", "Verifying the dev server starts". A long turn is read as a wall of steps without this, and the phases are what make it skimmable. Moving a plan step to in_progress already sets the phase, so with a plan you rarely need it; without one, use it every time the work genuinely changes.

## Plain text, not LaTeX
The transcript renders markdown, and maths notation is not what a code conversation needs. Write an arrow as → and not as $
ightarrow$ or 	o. Use ordinary Unicode for ≤ ≥ ≠ × ÷ ± and for Greek letters. Reserve $…$ / $$…$$ for genuine mathematics, where it is rendered properly.
...
**Every change must meet production quality:**

1. **Error Handling is Not Optional**
   - Handle all error cases explicitly (network failures, invalid input, missing data, race conditions)
   - Never let errors silently fail or crash the application
   - Provide meaningful error messages that help debug issues
   - Log errors appropriately with context for debugging
   - Use try-catch blocks, error boundaries (React), Result types (Rust), or equivalent patterns
   - Consider timeouts, retries with exponential backoff, and circuit breakers where appropriate

2. **Input Validation and Security**
   - Validate ALL user input and external data at system boundaries
   ...
   - Apply principle of least privilege for permissions and access control
   - Never commit secrets, API keys, or credentials to code

3. **Testing Must Be Comprehensive**
   - **Unit tests**: Test individual functions/methods in isolation with edge cases
   - **Integration tests**: Test how components work together
   - **Error path testing**: Write tests specifically for error conditions and edge cases
   - **Test organization**: Group related tests, use descriptive test names
   - **Test coverage goals**: Aim for >80% coverage on critical paths
   - **TDD when fixing bugs**: Write failing test → fix → verify test passes
   - Include tests for: empty arrays, null/undefined, concurrent access, boundary values, malformed input

4. **Code Quality and Maintainability**
   - Write self-documenting code with clear variable/function names
   - Add comments ONLY for non-obvious business logic or complex algorithms
   - Follow the project's existing patterns and conventions consistently
   - Keep functions small and focused (single responsibility)
   - Avoid premature optimization; prefer clarity first, then optimize if needed
   - Use TypeScript types strictly; avoid \`any\` (use \`unknown\` and type guards instead)
   - Handle async operations properly (no fire-and-forget without error handling)

5. **Performance Considerations**
   - Consider algorithmic complexity for data structures and operations
   - Avoid N+1 queries; batch database operations when possible
   - Implement pagination for large datasets
   - Use appropriate indexes for database queries
   - Cache expensive computations when appropriate
   - Monitor and set reasonable timeouts for external calls
   - Profile before optimizing; don't guess at bottlenecks

6. **Accessibility and UX**
   - Ensure UI is keyboard navigable and screen reader friendly
   - Use semantic HTML elements (\`button\`, \`nav\`, \`article\`, etc.)
   - Provide proper ARIA labels and roles where needed
   - Maintain sufficient color contrast ratios (WCAG AA minimum)
   - Handle loading states, empty states, and error states in UI
   - Provide user feedback for all actions (spinners, success/error messages)

7. **Logging and Observability**
   - Log critical operations and state changes at appropriate levels
   - Include relevant context in logs (user id, request id, timestamps)
   - Distinguish log levels: debug, info, warn, error
   - Never log sensitive information (passwords, tokens, PII)
   - Make errors traceable with stack traces and context

**When breaking down work:**
- Separate concerns: data model changes, business logic, UI, tests
- Each unit of work should be independently verifiable
- Infrastructure/setup tasks come before feature tasks that depend on them
- Don't stack "implement X and Y and Z" into one step—that's three steps
...
export async function runAgentLoop(params: {
  ...
  threadType?: ThreadType;
  specId?: string;
  /** What caused this run. Reported to the client so it can label the run and
   *  restore the Stop control for runs the user did not initiate. */
  trigger?: 'user' | 'watcher' | 'loop' | 'resume';
  /** Where the work happens. Recorded on the session the first time it is seen,
   *  so a reopened thread still knows its files are on another machine. */
  source?: import('../workspace/types').WorkspaceSource;
  onEvent: (event: WSServerEvent) => void;
}): Promise<void> {
  ...
  if (provider === 'openrouter' && !settings.openrouterApiKey) {
    ...
    params.onEvent({
      type: 'error',
      message: 'OpenRouter API key not configured. Please add your OpenRouter API key in Settings.',
      recoverable: true,
      suggestions: [
        'Open Settings and add your OpenRouter API key',
        'Get an API key from https://openrouter.ai/keys',
        'Alternatively, switch to Claude, Gemini, or Ollama in Settings',
      ],
    });
    return;
  }
  ...
  /*
   * Resolve GEMINI's real context window from the API.
   *
   * This was the one provider left guessing from a regex on the model name,
   * which is wrong in both directions and both are expensive: guess low and
   * every thread migrates to a fresh one after a dozen file reads, redoing work
   * forever; guess high and the run hits a hard provider error mid-turn, which
   * is exactly what context management exists to prevent.
   */
  if (agentConfig.provider === 'gemini' && agentConfig.geminiApiKey) {
    try {
      const { resolveGeminiContextLength } = await import('../models/gemini');
      const contextLength = await resolveGeminiContextLength(agentConfig.geminiApiKey, agentConfig.model);
      if (contextLength && contextLength > 0) {
        agentConfig.resolvedContextTokens = contextLength;
        logger.info('Resolved context window for Gemini model', {
          model: agentConfig.model, tokens: contextLength,
        });
      }
    } catch (err) {
      logger.warn('Could not resolve the Gemini context window; using the name-based estimate', {
        model: agentConfig.model, error: err instanceof Error ? err.message : String(err),
      });
    }
  }
  ...
  if (agentConfig.provider === 'openrouter' && agentConfig.apiKey) {
    try {
      const contextLength = await getOpenRouterModelContext(agentConfig.apiKey, agentConfig.model);
      if (contextLength && contextLength > 0) {
        agentConfig.resolvedContextTokens = contextLength;
        logger.info('Resolved context window for OpenRouter model', {
          model: agentConfig.model, tokens: contextLength,
        });
      }
    } catch (err) {
      logger.warn('Could not resolve OpenRouter model context window; using default', {
        model: agentConfig.model, error: err instanceof Error ? err.message : String(err),
      });
    }
  }
  ...
  if (!sessionId) {
    ...
    // Record the source BEFORE anything can run a tool against it.
    if (params.source && params.source.kind !== 'local') {
      try {
        const { setSessionSource, registerRemoteWorkspace } = await import('../workspace/registry');
        setSessionSource(sessionId, params.source);
        registerRemoteWorkspace(params.source);
      } catch (err) {
        logger.warn('Could not record the workspace source', {
  ...
  // Announce the run on the event stream itself, so EVERY client learns a run is
  // in flight regardless of how it was triggered — a user message, a watcher
  // wake-up, a loop tick, or the CLI. The Stop affordance is driven off this,
  // which is why a woken thread used to stream output with no way to interrupt.
  params.onEvent({ type: 'run_started', sessionId, trigger: params.trigger ?? 'user' });
...
User request: ` + userMessage;
    ...
    //
    // THE BUDGET SCALES WITH THE MODEL'S WINDOW.
    //
    // It used to be `min(contextTokenBudget, …)` with contextTokenBudget
    // defaulting to 24,000. On a 200k-window model that meant compaction fired
    // as soon as history passed 24k — about a dozen file reads — and kept firing,
    // shredding the working set while most of the window sat unused. The agent
    // then re-read files it had read minutes earlier, and long autonomous runs
    // went in circles.
    //
    // The setting is now a FLOOR, not a ceiling: use the larger of what the user
    // asked for and a healthy fraction of the real window, then clamp to
    // whatever actually fits below the migration line.
    const windowShare = Math.floor(usableWindow * 0.65);
    const headroom = Math.floor(migrationLineTokens - systemPromptTokens - 4000);
    const effectiveCompactionBudget = Math.max(
      Math.min(Math.max(contextTokenBudget, windowShare), headroom),
      4000,
    );
    logger.info('Context budget resolved', {
      model: agentConfig.model,
      windowTokens: operativeLimit.maxTokens,
      usableWindow,
      compactionBudget: effectiveCompactionBudget,
      migrationLine: Math.floor(migrationLineTokens),
    });
    ...
    /**
     * Hand the loop anything the user typed while it was working.
     *
     * Delivered as an ordinary user message, because that is what it is. The
     * framing tells the model how to weigh it: a mid-run instruction is almost
     * always a correction, and treating it as a polite aside is how an agent
     * carries on editing the wrong file for another ten steps.
     */
    const drainQueuedMessages = (): boolean => {
      const pending = takeQueuedMessages(sessionId!);
      if (pending.length === 0) return false;
      for (const text of pending) {
        saveMessage(sessionId!, 'user', text);
        params.onEvent({ type: 'queued_message_delivered', sessionId, message: text } as never);
        messages.push({
          role: 'user',
          content:
            `[The user sent this WHILE you were working. It is a new instruction and it takes priority over ` +
            `whatever you were about to do next. If it changes the goal, say so in one line and change course; ` +
            `if it only adds to it, fold it into your plan. Do not silently ignore it.]\n\n${text}`,
        });
      }
      logger.info('Delivered queued user messages mid-run', { sessionId, count: pending.length });
      return true;
    };

    /**
     * Watchers that settled between the agent registering them and the turn
     * actually ending.
     *
     * `watch(detached:true)` promises "end your turn, you'll be resumed". The
     * resume is fired by the settle listener, which stands down while the
     * thread is still running — so a wait that finishes inside the couple of
     * seconds between the tool returning and the model stopping settled with
     * nobody left to hear it, and the thread parked forever waiting for a
     * wake-up that had already happened. Draining here closes that window: the
     * turn does not end while an unread result is sitting on the table.
     */
    const drainSettledWatchers = (): boolean => {
      let done: Array<{ label: string; outcome: string; detail: string; output?: string; exitCode?: number | null }>;
      try {
        done = watchers.collectUndelivered(sessionId).filter((w) => w.outcome !== 'cancelled');
      } catch { return false; }
      if (done.length === 0) return false;
      const body = done.map((w) => {
        const parts = [`Watching: ${w.label}`, `Outcome: ${w.outcome.toUpperCase()} — ${w.detail}`];
        if (w.exitCode != null) parts.push(`Exit code: ${w.exitCode}`);
        if (w.output) parts.push(`Output:\n${w.output}`);
        return parts.join('\n');
      }).join('\n\n');
      messages.push({
        role: 'user',
        content:
          `[automatic wake-up, delivered before your turn ended] ${done.length} wait(s) you registered finished ` +
          `while you were still working, so you are being told now rather than being resumed later.\n\n${body}\n\n` +
          `Continue from here. If something failed, diagnose it from the output above rather than re-running it blindly.`,
      });
      logger.info('Delivered watcher results before ending the turn', { sessionId, count: done.length });
      return true;
    };
    ...
    while (iteration < MAX_ITERATIONS && !stopped) {
      ...
      try {
        const stateBlock = buildRuntimeStateBlock({ workspacePath: params.workspacePath, specId, sessionId });
      ...
      if (response.toolCalls.length === 0) {
        ...
        // Before the turn ends: anything that arrived WHILE it was running.
        //
        // Both of these are messages the loop already has but has not shown the
        // model. Ending the turn on top of them is how "I told it mid-run and
        // it ignored me" and "the watcher fired but nothing woke up" happen.
        if (drainQueuedMessages()) continue;
        if (drainSettledWatchers()) continue;
      ...
      for (const toolCall of response.toolCalls) {
        ...
        if (approvalCheck.required) {
          ...
          // Wait for a human decision (see APPROVAL_TIMEOUT_MS).
          ...
          // fire (and hold the event loop) half an hour later.
          ...
          logAuditEvent({
            ...
            resultSummary: approved
              ...
              : timedOut
              ? `Expired — no response within ${Math.round(APPROVAL_TIMEOUT_MS / 60_000)} minutes`
        ...
        try {
          ...
          const toolOnEvent = (event: { type: string; content: string }) => {
            ...
            if (event.type === 'terminal_start' || event.type === 'terminal_output' || event.type === 'terminal_end') {
              ...
              // "What I am doing now", in the agent's own words. The transcript
              // segments a burst of tool calls by these, which is what turns
              // twenty steps into "built the backend, found the failure, fixed
              // it, verified" — see ToolStepGroup.
              try {
                const p = JSON.parse(event.content) as { label: string; detail?: string; source?: 'agent' | 'plan' };
                if (p?.label) params.onEvent({ type: 'phase', label: p.label, detail: p.detail, source: p.source ?? 'agent' });
              } catch { /* a malformed phase is simply not shown */ }
      ...
      // A mid-run instruction is delivered at the FIRST boundary after it
      // arrives, not only at the end of the turn — otherwise "stop, wrong
      // file" waits out another fifteen tool calls before the agent reads it.
      drainQueuedMessages();
