...
export function buildRuntimeStateBlock(opts: RuntimeStateOptions): string {
  ...
  // --- Detached watchers ---------------------------------------------------
  try {
    const live = watchers.list().filter((w) => !w.settled);
    const done = watchers.list().filter((w) => w.settled && w.outcome);
    if (live.length > 0) {
      sections.push(
        `### Waiting on\n` +
        live.map((w) => `- ${w.label} (${w.id}, ${Math.round(w.ageMs / 1000)}s so far)`).join('\n') +
