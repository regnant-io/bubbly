...
import { isAbsolutelyBlocked, isDestructiveCommand, planSpawn } from './shell';
...
interface BackgroundProcess {
  ...
  /** The dev-server URL this process is CURRENTLY believed to serve. */
  detectedUrl: string | null;
  /**
   * Every distinct address this process has announced, in the order it said them.
   *
   * A dev server that finds its port taken says so and moves: "Port 3000 is in
   * use, trying 3001". Recording only the first sighting meant Bubbly — and the
   * agent reading Bubbly's state — believed the server was on 3000 forever,
   * while the thing actually serving was on 3001. The agent would then read the
   * logs, notice the real port itself, and be left holding two contradictory
   * facts with no way to tell which was current. Keeping the history and
   * treating the LATEST sighting as the truth makes the move visible instead of
   * invisible, and the list is short enough to hand to the model verbatim.
   */
  urlSightings: string[];
  /** Fired whenever the believed address CHANGES, including the first time. */
...
/** The same pattern, global — used to collect EVERY address a chunk announces,
 *  which is how a "port 3000 is in use, trying 3001" line is understood. */
const DEV_SERVER_URL_RE_G = new RegExp(DEV_SERVER_URL_RE.source, 'gi');
...
class BackgroundProcessManager {
  ...
  private append(p: BackgroundProcess, chunk: string): void {
    ...
    // preview manually.
    {
      ...
      const found = clean.match(DEV_SERVER_URL_RE_G) ?? (p.detectedUrl ? null : stripAnsi(p.output).match(DEV_SERVER_URL_RE_G));
      for (const raw of found ?? []) {
        const url = raw.replace(/[.,;]+$/, '');
        if (p.urlSightings[p.urlSightings.length - 1] === url) continue;
        if (!p.urlSightings.includes(url)) p.urlSightings.push(url);
        if (p.urlSightings.length > 8) p.urlSightings.shift();
        if (p.detectedUrl === url) continue;
        const previous = p.detectedUrl;
        p.detectedUrl = url;
        logger.info('Dev-server URL detected in background process output', {
          id: p.id, url, previous: previous ?? undefined,
        });
        p.onUrlDetected?.(url);
      }
    }
  ...
  start(
    ...
    // Same per-command shell routing as the one-shot runner, so a command
    // cannot behave differently just because it was backgrounded.
    //
    // Deliberately NOT non-interactive here (unlike the one-shot shell): some
    // dev servers change behaviour under CI — react-scripts turns warnings into
    // errors and refuses to start — and a background process CAN be answered
    // later via send_process_input, so it doesn't need that hard stance.
    ...
    try {
      proc = spawn(plan.file, plan.argv, {
        cwd,
        env: plan.env,
        windowsHide: true,
        // Non-negotiable for cmd.exe: without it Node re-escapes the command's
        // quotes in a form cmd cannot read, which corrupts the command AND
        // reports its failure as exit code 0. See shellDialect.shellArgv.
    ...
    const p: BackgroundProcess = {
      ...
      awaitingInput: null, detectedUrl: null, urlSightings: [], onUrlDetected, listeners: new Set(), settled: false,
    };
    this.procs.set(id, p);
  ...
  list(): Array<{ id: string; command: string; cwd: string; status: string; exitCode: number | null; uptimeMs: number; awaitingInput: boolean; detectedUrl: string | null; urlSightings: string[]; startedAt: number }> {
