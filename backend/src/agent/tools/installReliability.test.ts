...
import { resolveCommandCwd, verifyInstall, installedPackageNames } from './shell';
...
// The chain-operator behaviour now lives in shellNormalize.test.ts, where it is
// tested per DIALECT. Rewriting `&&` is correct for PowerShell 5.1 and wrong for
// cmd.exe, so a single unconditional expectation (which is what used to live
// here) cannot be right for both.
