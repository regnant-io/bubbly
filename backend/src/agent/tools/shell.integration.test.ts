...
import { findWindowsPowerShell, findPwsh } from './shellDialect';
import os from 'os';
import fs from 'fs';
...
describe('Shell Integration Tests', () => {
  ...
  describe('Windows PowerShell Integration', () => {
    ...
    it('should handle PowerShell cmdlets', () => {
      ...
      // Routed to PowerShell by its syntax. If this machine genuinely has no
      // PowerShell, the command is correctly refused by cmd.exe instead — the
      // assertion is about ROUTING, so it must not depend on what is installed.
      const result = runShell('Get-Date', testWorkspace);
      if (findWindowsPowerShell() || findPwsh()) {
        expect(result.exitCode).toBe(0);
    ...
    it('should provide Windows alternatives for Unix commands', () => {
      ...
      if (result.exitCode === 1 && result.stderr.includes('not recognized')) {
        ...
        expect(result.stderr).toContain('findstr');
      }
    });
  });
