...
/**
 * The REAL context window of a Gemini model, from the API.
 *
 * Bubbly resolved Ollama's window from `/api/show` and OpenRouter's from its
 * models endpoint, but guessed Gemini's from a regex on the model name. That
 * guess is wrong in both directions and both are costly: too low and every
 * thread migrates to a fresh one after a dozen file reads, redoing work
 * forever; too high and the run hits a hard provider error mid-turn, which is
 * the one failure mode context management exists to prevent.
 *
 * Google publishes it as `inputTokenLimit` on the model resource. Cached per
 * model, because it does not change and a run makes this call every time it
 * starts.
 */
const geminiContextCache = new Map<string, number>();

export async function resolveGeminiContextLength(
  apiKey: string,
  model: string,
  baseUrl?: string,
): Promise<number | null> {
  ...
  const key = `${baseUrl ?? DEFAULT_BASE_URL}::${model}`;
  const cached = geminiContextCache.get(key);
  if (cached) return cached;

  try {
    const name = model.startsWith('models/') ? model : `models/${model}`;
    const url = `${(baseUrl || DEFAULT_BASE_URL).replace(/\/$/, '')}/v1beta/${name}`;
    const res = await fetch(url, {
      headers: { 'x-goog-api-key': apiKey },
      signal: AbortSignal.timeout(8000),
    });
    if (!res.ok) {
      logger.warn('Could not resolve the Gemini context window; falling back to the name-based estimate', {
        model, status: res.status,
      });
      return null;
    }

    const data = (await res.json()) as { inputTokenLimit?: number; outputTokenLimit?: number };
    const limit = Number(data.inputTokenLimit);
    if (!Number.isFinite(limit) || limit <= 0) return null;

    geminiContextCache.set(key, limit);
    logger.info('Resolved the Gemini context window', { model, inputTokenLimit: limit });
    return limit;
  } catch (err) {
    logger.warn('Could not resolve the Gemini context window', {
