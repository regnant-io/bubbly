...
export async function callOllama(params: {
  ...
  if (params.onToken && response.body) {
    ...
    /*
     * ONE CALL MUST BE ANNOUNCED ONCE, HOWEVER MANY CHUNKS CARRY IT.
     *
     * `message.tool_calls` is not a delta — Ollama (and anything proxying it)
     * may repeat the SAME call on several NDJSON lines as generation finishes,
     * and the leak filter can re-recover a call already adopted. Every emission
     * used to mint a fresh `ollama_tc_<i>_<Date.now()>` id, so the client saw a
     * new call each time and drew a new spinner: one browser_control call
     * became thirty "Using the browser…" rows, none of which ever resolved
     * because only one of them was ever executed.
     *
     * Identity is the (name, args) pair — the only thing that distinguishes one
     * call from another when the transport gives us no id of its own. A model
     * genuinely asking for the exact same tool with the exact same arguments
     * twice in ONE response is asking for the same work twice; collapsing that
     * is right too.
     */
    const seenToolCalls = new Set<string>();
    const signatureOf = (name: string, args: Record<string, unknown>): string => {
      try { return `${name}:${JSON.stringify(args)}`; } catch { return `${name}:[unserialisable]`; }
    };
    /** Register a call, announce it, and keep it — or ignore a repeat. */
    const acceptToolCall = (name: string, args: Record<string, unknown>, idPrefix: string): boolean => {
      const signature = signatureOf(name, args);
      if (seenToolCalls.has(signature)) return false;
      seenToolCalls.add(signature);
      const id = `${idPrefix}_${toolCalls.length}_${Date.now()}`;
    ...
    const adoptLeakedCalls = () => {
      for (const call of leakFilter.recovered.splice(0)) {
        if (!acceptToolCall(call.name, call.args, 'ollama_tc_leak')) continue;
      ...
      if (data.message) {
        ...
        if (data.message.tool_calls) {
          for (let i = 0; i < data.message.tool_calls.length; i++) {
            ...
            //
            // acceptToolCall drops a repeat of a call we have already announced:
            // the same call can arrive on several chunks, and announcing it
            // twice draws two spinners for one piece of work.
            acceptToolCall(tc.function.name, args as Record<string, unknown>, 'ollama_tc');
          }
        }
      }
      if (data.done) {
    ...
    if (toolCalls.length === 0 && looksLikeBareToolCall(textContent)) {
      ...
      if (rescued) {
        ...
        acceptToolCall(rescued.name, rescued.args, 'ollama_tc_bare');
        textContent = '';
      }
    }
