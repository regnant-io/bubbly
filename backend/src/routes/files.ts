...
function getWorkspace(query: Record<string, unknown>): string {
  // Accept BOTH spellings. The chat transcript's file chips call this with
  // `workspacePath` (the name used everywhere in the WebSocket protocol and the
  // store), while the file explorer calls it with `workspace`. Honouring only
  // one of them meant clicking a file in the conversation fell back to the
  // global workspace setting and then 400'd on the missing `path`, which the UI
  // reported as "this file could not be read — it may have been deleted".
  const ws = (query.workspace as string) || (query.workspacePath as string) || getAllSettings().workspacePath;
