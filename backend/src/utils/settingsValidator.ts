...
export async function validateSettings(
  ...
  if (updates.openrouterApiKey !== undefined) {
    const apiKey = updates.openrouterApiKey.trim();
    // Test OpenRouter API key validity if it's being changed and is not empty
    if (apiKey && apiKey !== currentSettings.openrouterApiKey) {
      logger.info('Testing OpenRouter API key validity');
      const isValid = await testOpenRouterApiKey(apiKey);
      if (!isValid) {
        errors.push({
          field: 'openrouterApiKey',
          message: 'API key is invalid or cannot connect to OpenRouter API',
        });
      }
    }
  }
...
/**
 * Test OpenRouter API key validity by checking models endpoint
 */
async function testOpenRouterApiKey(apiKey: string): Promise<boolean> {
  try {
    ...
    logger.warn('OpenRouter API key validation failed', { status: response.status });
    return false;
  } catch (error) {
    logger.error('OpenRouter API key validation error', {
      error: error instanceof Error ? error.message : String(error),
    });
    return false;
  }
