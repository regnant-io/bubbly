...
import { Eye, ListTree, Image as ImageIcon, Table as TableIcon } from '../Shared/icons';

/** How the pane should present the file currently open in the editor. */
export type PreviewMode = 'rendered' | 'outline';
 ...
 *
 * TWO MODES, BECAUSE "PREVIEW" MEANS TWO THINGS
 *
 * A markdown file, an image or a CSV has a rendered form. A source file does
 * not — the only useful overview it has is its symbol outline.
 *
 * There used to be a third tab, "App", which embedded the whole Bubbly Preview
 * beside the editor. It was removed: the preview is a full panel in the right
 * stack with its own lifecycle, its own toolbar and its own tabs, and squeezing
 * it into a column beside a code editor gave a cramped, half-usable copy of
 * something already available at full size. Two things doing the same job, one
 * of them badly, is worse than one thing doing it properly.
 *
 * The pane is HIDDEN by default. Most editing needs no preview, and a pane that
 * opens itself halves the width of the editor to show an outline nobody asked
 * for.
 */
