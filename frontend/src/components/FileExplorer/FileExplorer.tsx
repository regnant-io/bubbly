...
export function FileExplorer() {
  ...
  /*
   * Subscribed to purely so the tree RE-RENDERS when the theme flips.
   *
   * File icons take their colour from the language convention (TypeScript blue,
   * Ruby dark red), adjusted for legibility against the current background —
   * see `legibleColor` in fileIcons. That adjustment is read at render time, so
   * without a dependency on the resolved theme the icons keep their old,
   * now-invisible colours until something else happens to re-render the tree.
   */
  ...
  return (
    // `data-icon-theme` — NOT `data-theme` — so the subscription above is a real
    // dependency without colliding with the palette selectors, which match on
    // `data-theme` and would redefine every token for this subtree.
    <div className="flex flex-col h-full" data-icon-theme={resolvedTheme}>
      <PanelHeader
                ...
                return (
                  ...
                  >
                    ...
                    {Array.from({ length: depth }).map((_, k) => (
                      <span key={k} className="shrink-0 self-stretch w-3 border-l border-hairline/25" aria-hidden />
                    ))}
