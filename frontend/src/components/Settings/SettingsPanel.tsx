...
import { fetchSettings, fetchOllamaModels, fetchOllamaStatus, fetchGeminiModels, fetchModelContext, type ResolvedContext } from '../../hooks/useApi';
...
type CategoryId = 'general' | 'providers' | 'agent' | 'editor' | 'mcp' | 'skills' | 'safety';
                  ...
                  <Field label="Provider">
                    <div className="flex gap-3">
                      {(['claude', 'ollama', 'gemini'] as const).map((p) => (
                        ...
                        >
                          {p === 'claude' ? <Zap size={14} /> : p === 'gemini' ? <Sparkles size={14} /> : <Cpu size={14} />}
...
function Toggle({ label, hint, checked, onChange }: { label: string; hint?: string; checked: boolean; onChange: (v: boolean) => void }) {
  return (
    <div className="flex items-start justify-between gap-4">
      ...
      >
        <span className={`inline-block h-4 w-4 rounded-full shadow-md transform transition-transform ${
          checked ? 'translate-x-[22px] bg-white' : 'translate-x-1 bg-text-dim'
        }`} />
      </button>
    </div>
  );
