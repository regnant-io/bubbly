...
/** One service from the project's run config, and whether it is up. */
export interface PreviewService {
  name: string;
  kind: string;
  running: boolean;
  processId: string | null;
  /** The address it is actually serving on, once detected. Null until then. */
  url: string | null;
}
 ...
 *
 * `services` is the per-service breakdown. A project with a frontend and an API
 * has two, and the preview offers a tab for each — the top-level `url` is only
 * the primary one, which for a full-stack project is half the answer.
 */
export async function previewServerStatus(workspacePath: string): Promise<{
  running: boolean;
  url: string | null;
  note?: string | null;
  processId?: string;
...
export async function fetchOpenRouterModels() {
  const res = await fetch(`${BASE}/settings/openrouter/models`);
  return res.json();
}
