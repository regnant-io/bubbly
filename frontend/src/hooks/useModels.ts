...
import { fetchOllamaModels, fetchGeminiModels, saveSettings, fetchModelVision } from './useApi';
...
export function useModels() {
  ...
  useEffect(() => {
    ...
    // Claude/Gemini are always vision; skip the round-trip.
  ...
  const refresh = useCallback(async () => {
    setLoading(true);
    try {
      const [oll, gem] = await Promise.allSettled([fetchOllamaModels(), fetchGeminiModels()]);
  ...
  const selectModel = useCallback(
    async (opt: ModelOption) => {
      if (!settings) return;
      const field = opt.provider === 'claude' ? 'claudeModel' : opt.provider === 'gemini' ? 'geminiModel' : 'ollamaModel';
