...
export function useWebSocket() {
  ...
  const handleEvent = useCallback((event: WSServerEvent) => {
    ...
    switch (event.type) {
      ...
      case 'run_started':
        // A run is in flight — however it started. The Stop control keys off
        // this, which is what a watcher wake-up used to be missing entirely.
        store.beginRun(event.trigger);
        if (event.trigger === 'watcher') {
          store.addMessage({
            id: nanoid(),
            type: 'status',
            content: event.detail
              ? `Resumed automatically: ${event.detail}`
              : 'Resumed automatically after a background wait finished.',
            timestamp: Date.now(),
          });
        }
        break;

      case 'watchers_updated':
        store.setWatchers(event.watchers);
        break;

      case 'phase':
        // Nothing is drawn for this on its own — it is stamped onto every tool
        // call made from here on, and the transcript groups a burst by it.
        store.setCurrentPhase({ label: event.label, detail: event.detail, source: event.source });
        break;

      // Messages typed while the agent was already working.
      case 'message_queued':
        store.enqueuePendingMessage(event.message, event.depth);
        break;

      case 'message_queue_rejected':
        store.rejectPendingMessage(event.message, event.reason);
        break;

      case 'queued_message_delivered':
        // The agent has now actually read it, so it becomes a real user bubble
      ...
      case 'loop_started':
        store.setActiveLoop({
          ...
          remainingMinutes: event.maxMinutes,
        });
        break;
      ...
      case 'loop_iteration':
        store.setActiveLoop({
          loopId: event.loopId,
          goal: useStore.getState().activeLoop?.goal ?? '',
          iteration: event.iteration,
          maxIterations: event.maxIterations,
          maxMinutes: useStore.getState().activeLoop?.maxMinutes ?? 0,
          remainingMinutes: event.remainingMinutes,
        });
        break;

      case 'loop_finished':
        store.setActiveLoop(null);
        store.addMessage({
          id: nanoid(),
          type: 'status',
      ...
      case 'approval_required':
        ...
        // when they've switched away — an unanswered approval eventually expires.
        void notifyDesktop({
      ...
      case 'approval_timeout':
        // The backend declined the action after nobody answered in time —
  ...
  const sendChat = useCallback(
    (message: string, workspacePath: string, sessionId?: string | null, threadType?: string, specId?: string) => {
      ...
      sendMessage({
        ...
        threadType,
        specId,
        // Where the work happens. Only meaningful on the first message of a
        // thread — after that the session already has it on record — but sent
        // every time so a reconnect mid-thread cannot lose it.
        source: store.workspaceSource ?? undefined,
      });
    },
    [sendMessage]
  );
  ...
  /**
   * Run a slash-command workflow.
   *
   * Only the command NAME and its arguments cross the wire — the expansion into
   * a real prompt happens on the server, so the desktop app and the CLI cannot
   * end up with different ideas of what `/fix` does. `label` is what appears in
   * the transcript, which is the short thing the user typed rather than the
   * program it became.
   */
  const sendWorkflow = useCallback(
    (
      ...
      shownLenRef.current = 0;
      streamedThisTurnRef.current = '';
      store.addMessage({ id: nanoid(), type: 'user', content: label, timestamp: Date.now() });
      ...
      sendMessage({
        ...
        workflow: {
          ...workflow,
          // Open editor tabs are the only signal for a request like "/fix this".
          openFiles: store.editorTabs.map((t) => t.path).slice(0, 8),
        },
      });
    },
    [sendMessage],
   ...
   *
   * The run is single-flight on the server, so this cannot be an ordinary
   * `chat` — that would be refused. It parks the text against the thread and
   * the running loop picks it up at its next step boundary. The optimistic
   * chip appears immediately; the real user bubble appears when the agent
   * actually reads it, at the point in the transcript where that happened.
   */
  const sendQueuedMessage = useCallback(
    (sessionId: string, message: string) => {
      ...
      const store = storeRef.current;
      if (store.pendingMessages.filter((m) => m.status === 'queued').length >= 3) return false;
      store.enqueuePendingMessage(text);
      sendMessage({ type: 'queue_message', sessionId, message: text });
      return true;
    },
    [sendMessage]
  );

  /** Stop waiting on one watcher without stopping the turn. */
  const sendSkipWatch = useCallback(
    (watcherId: string) => { sendMessage({ type: 'skip_watch', watcherId }); },
    [sendMessage]
  );
  ...
  return {
    sendChat, sendWorkflow, sendApprove, sendReject, sendStop,
    sendQueuedMessage, sendSkipWatch,
