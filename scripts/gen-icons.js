 ...
 *
 * Renders the Bubbly bubble mark (same purple bubble used as the in-app favicon,
 * on a dark rounded square) and writes:
...
function drawIcon(px, finalSize) {
  ...
  // Dark rounded-square background (#1a1614).
  roundedRect(c, [0x1a, 0x16, 0x14, 255], Math.max(1, Math.round(56 * s)));

  // Purple radial-gradient bubble: center #a78bfa -> edge #5b21b6.
  const bubbleCx = 128 * s;
  const bubbleCy = 128 * s;
  const bubbleR = 92 * s;
  const inner = [0xa7, 0x8b, 0xfa];
  const outer = [0x5b, 0x21, 0xb6];
  // gradient focus offset toward top-left (40%, 35%)
  const focusX = bubbleCx - bubbleR * 0.2;
  ...
  // Highlight reflections. The large one reads at every size; the small one
  // turns to mud below 32px, so it's dropped there.
  filledCircle(c, 98 * s, 100 * s, 20 * s, () => [255, 255, 255, 64]);
  if (finalSize >= 32) {
    filledCircle(c, 86 * s, 86 * s, 10 * s, () => [255, 255, 255, 128]);
  }
